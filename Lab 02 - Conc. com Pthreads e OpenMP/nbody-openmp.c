...
int main()
{
    struct timeval inicioTudo, finalTudo;
    ...
    tmili = (int) (1000 * (finalTudo.tv_sec - inicioTudo.tv_sec) +
...
#pragma omp parallel for private(xi, yi, rx, ry, mj, r) reduction(-:fx, fy) num_threads(2)
    for (i=0; i<npart; i++) {
        rmin = 100.0;
        xi   = myparticles[i].x;
        yi   = myparticles[i].y;
...
#pragma omp parallel for private(xi, yi, i) num_threads(2)
    for (i=0; i<npart; i++) {
        double xi, yi;
        xi	           = particles[i].x;
        yi	           = particles[i].y;
